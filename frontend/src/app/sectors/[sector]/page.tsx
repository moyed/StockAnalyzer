...
function getRecommendation(avg: number | null) {
  if (avg === null)
    return {
      ...
      rationale:
        "This sector lacks sufficient AI-analysed filings to make a recommendation.",
    };
  if (avg >= 70)
    return {
      label: "Strong Buy",
      cls: "bg-green-700 text-white",
      rationale: `With an average score of ${avg}, this sector shows exceptional financial performance. Companies demonstrate strong revenue growth, profit expansion, and positive business momentum. Consider increasing allocation.`,
    };
  if (avg >= 55)
    return {
      label: "Buy",
      cls: "bg-green-500 text-white",
      rationale: `An average score of ${avg} indicates solid fundamentals across this sector. Above-average performance suggests a favourable risk-reward profile — suitable for medium-to-long-term investors.`,
    };
  if (avg >= 40)
    return {
      label: "Hold",
      cls: "bg-yellow-400 text-yellow-900",
      rationale: `This sector averages ${avg} — mixed signals. Some companies show strength while others face headwinds. Selective stock-picking is advised; avoid broad sector bets.`,
    };
  if (avg >= 25)
    return {
      label: "Underweight",
      cls: "bg-orange-500 text-white",
      rationale: `With an average score of ${avg}, this sector is underperforming. Caution is warranted. Consider reducing exposure or waiting for clear recovery signals before adding positions.`,
    };
  return {
    label: "Avoid",
    cls: "bg-red-600 text-white",
    rationale: `A score of ${avg} signals significant weakness across this sector. Multiple negative indicators present. Avoid new positions until fundamentals show clear improvement.`,
  };
}
...
export default function SectorDetailPage({
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["sector-companies", decodedSector],
    queryFn: () =>
      api
        .get(
          `/companies?sector=${encodeURIComponent(decodedSector)}&sort=score&per_page=500`
        )
        .then((r) => r.data),
    staleTime: 5 * 60_000,
  });
  ...
  const buckets = [
    {
      range: "0–19",
      count: scores.filter((s) => s < 20).length,
      color: "#dc2626",
    },
    {
      range: "20–39",
    ...
    {
      range: "80–100",
      count: scores.filter((s) => s >= 80).length,
      color: "#15803d",
    },
  ];
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{decodedSector}</h1>
          <p className="text-gray-500 text-sm">
            Sector analysis · {companies.length} companies
          </p>
        </div>
        <span
              ...
              <div>
                ...
                {defaulterCount > 0 && (
                  <p className="text-xs text-red-600 mt-2">
                    ...
                    in this sector — screen individual companies carefully.
                  </p>
                )}
              </div>
            </div>
